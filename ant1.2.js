...
var ant = function(){
    ...
    */
    ...
    /*
        ...
        如果中间件不返回值则表示到此为止，不会继续传递到下一层中间件。
        返回值必须为res表示成功执行，否则就被认为是失败。
        在中间件中，要保证处理是同步的，如果需要用到异步的处理，
        最好是使用Promise或是其它方案设计成同步的模式。
        中间件经常用于权限管理，会话验证，请求过滤等，这些都是同步的操作，不需要异步模式。
        不要把此中间件模式和其他框架的中间件混淆，此模式就是在请求过来以后进行的过滤处理。
    */
    this.middleware = [];
    ...
    this.usemiddle = function(mid) {
        if (typeof mid.middleware === 'object') {
            if (typeof mid.middleware.mid === 'function') {
                ...
                var preg = '';
                var name = '';
                if (typeof mid.middleware.fail === 'function') {
    ...
    this.runMiddleware = function(req, res) {
        ...
        for (var i=0; i < this.middleware.length; i++) {
            if (typeof this.middleware[i].method === 'function') {
                ...
                ret = this.middleware[i].method(req, res);
                if (ret !== res) {
                    return {
                        status : false,
                        middle : this.middleware[i]
                    };
                }
            }
        }
