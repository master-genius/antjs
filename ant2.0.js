...
var ant = function(){
    ...
    */
    ...
    this.ants = function(host='127.0.0.1', port=2018, options = {num : 0, version : 1}) {
    ...
    /*
        HTTP/2协议的支持，目前这是一个测试的功能，尽管node提供了完整的模块支持，
        但是实际的部署还是建议使用Nginx作为HTTPS/HTTP2服务器反向代理至node，
        而node只需要HTTP的支持即可。
    */
    this.run2 = function(host, port) {
        
        try {
            var opts = {
                key  : fs.readFileSync(this.config.http2_options.key),
                cert : fs.readFileSync(this.config.http2_options.cert)
            };
        } catch (err) {
            console.log(err);
            return false;
        ...
        const serv = http2.createSecureServer(opts);
